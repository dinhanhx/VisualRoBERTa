...
from src.data import PretrainTask, PretrainCollator
...
class Wrapper(pl.LightningModule):
    def __init__(self, model_config, warmup_ratio: float, learn_rate: float, use_phobert: bool) -> None:
        super().__init__()
        ...
        if use_phobert:
            self.model.imagetext.econder.load_state_dict(torch.load('assets/phobert-base-encoder.pt'))
...
if '__main__' == __name__:
    seed_everything(5)
    pretrain_task = PretrainTask(Path('/home/dinhanhx/data'),
    ...
    pretrain_collator = PretrainCollator(bun_tokenizer,
                                         image_size=config.image_size,
                                         patch_size=config.patch_size)
    sampler = DistributedSampler(
            pretrain_task, num_replicas=xm.xrt_world_size(), rank=xm.get_ordinal(), shuffle=False
        )
    dataloader = DataLoader(pretrain_task,
                            batch_size=8,
                            sampler=sampler,
                            num_workers=24,
    ...
    trainer = Trainer(logger=[CSVLogger("logs"), TensorBoardLogger("logs")],
                      max_epochs=2,
